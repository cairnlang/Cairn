...
Concurrency examples live under `examples/concurrency/`; `ping_pong_types.ax` and `traffic_light_types.ax` stay type-focused, while `ping_once.ax`, `self_boot.ax`, `two_pings.ax`, and `counter.ax` exercise the current runtime actor path (`counter.ax` demonstrates stack-carried actor state across repeated receives).
...
- **v0.7.0f** (current): Stateful actor example using stack-carried state
