...
# Tag-bound + helper-comparison constructor narrowing demo (new v0.6.0z slice)
...
- **v0.6.0z** (current): PROVE extracts tag constraints from helper-comparison encodings (`ite` boolean tags) and emits `inference_source` in JSON trace
- **v0.6.x** (next): Broaden helper-pattern inference coverage beyond current bounded encodings
