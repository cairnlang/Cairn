...
Concurrency examples live under `examples/concurrency/`; `ping_pong_types.ax` and `traffic_light_types.ax` stay type-focused, while `ping_once.ax`, `self_boot.ax`, `two_pings.ax`, `counter.ax`, `traffic_light.ax`, and `notifier.ax` exercise the current runtime actor path (`notifier.ax` is the first more practical actor-shaped workflow). Shared actor/state helpers now live under `examples/concurrency/lib/` (`lib/actor.ax`, `lib/state.ax`). Lifecycle-only examples like `examples/concurrency/linked_failure.ax` intentionally terminate the linked caller and are kept out of the normal runnable examples list.
...
- **v0.7.0j** (current): Linked actor lifecycle basics (`SPAWN_LINK`, actor-only `EXIT`)
- **v0.7.0** (next): Typed BEAM concurrency runtime completion (restart/supervision ergonomics, richer actor patterns)
