...
Concurrency examples live under `examples/concurrency/`; `ping_pong_types.ax` and `traffic_light_types.ax` stay type-focused, while `ping_once.ax` and `self_boot.ax` exercise the current runtime actor path (`self_boot.ax` demonstrates `SELF`).
...
- **v0.7.0c** (current): Minimal typed-concurrency runtime extended with `SELF`
