...
Concurrency examples live under `examples/concurrency/`; `ping_pong_types.ax` and `traffic_light_types.ax` stay type-focused, while `ping_once.ax` and `self_boot.ax` exercise the current runtime actor path (`self_boot.ax` demonstrates `SELF` through a helper function).
...
- **v0.7.0d** (current): Actor-context-aware `SELF` helper calls
- **v0.7.0** (next): Typed BEAM concurrency runtime completion (richer receive loops, supervision ergonomics)
