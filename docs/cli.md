...
`examples/web/hello_static.crn` is the first transport milestone: a deliberately tiny static HTTP server that listens on `127.0.0.1:8089`, passes each request path into a Cairn handler block, and returns `index.html` for `GET /`. The host runtime owns the accept loop and HTTP framing; Cairn owns the route choice and response content.
