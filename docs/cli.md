...
- IR export summary is on stderr:
  - success: `IR SUMMARY: status=ok functions=<n> elapsed_ms=<ms>`
...
7. Run the typed policy-engine showcase:
...
8. Run the first bounded web-serving example:
   - `./cairn examples/web/hello_static.crn`
   - `CAIRN_DATA_STORE_BACKEND=postgres ./cairn examples/web/todo_app.crn` (or omit the env var for `mnesia`)
