...
`examples/web/hello_static.crn` is the first transport milestone: a deliberately tiny static HTTP server that serves one request on `127.0.0.1:8089`, passes the request path into a Cairn handler block, returns `index.html` for `GET /`, and then exits. It is intentionally bounded so the transport path is real and testable before Cairn grows into a longer-lived web server.
