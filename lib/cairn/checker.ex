defmodule Cairn.Checker do
  @moduledoc """
  Static type checker for Cairn.
  ...
  of `Cairn.Checker.Error` structs.
    ...
    {body_actor_type, body_protocol_steps, body_types, body_env} =
      cond do
        is_list(helper_protocol_steps) and helper_protocol_steps != [] ->
          ...
          helper_typedef = %Cairn.Types.TypeDef{name: helper_type_name, variants: helper_variants}
    ...
    case state.current_protocol_steps do
      [{:recv, expected_ctor} | rest_steps] ->
        ...
        case {Map.get(state.types, type_name), expected_arm} do
          {%Cairn.Types.TypeDef{} = typedef, {^expected_ctor, arm_tokens}} ->
  ...
  defp compute_actor_required_functions(items, type_env) do
    functions =
      Enum.reduce(items, %{}, fn
        %Cairn.Types.Function{} = func, acc -> Map.put(acc, func.name, func)
        _, acc -> acc
      end)
