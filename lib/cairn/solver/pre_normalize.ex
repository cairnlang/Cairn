defmodule Cairn.Solver.PreNormalize do
  @moduledoc """
  ...
  `Cairn.Solver.Prove` is resilient to noisy/generated boolean structure.
  """
  ...
  def normalize_with_rewrites(term) do
    previous = Process.get(:cairn_pre_normalize_rewrites, :unset)
    Process.put(:cairn_pre_normalize_rewrites, [])
    normalized = normalize(term)
    rewrites = Process.get(:cairn_pre_normalize_rewrites, []) |> Enum.reverse()

    case previous do
      :unset -> Process.delete(:cairn_pre_normalize_rewrites)
      _ -> Process.put(:cairn_pre_normalize_rewrites, previous)
    end
  ...
  defp maybe_record_rewrite(rule, before_term, after_term) do
    case Process.get(:cairn_pre_normalize_rewrites, :disabled) do
      events when is_list(events) ->
        event = %{rule: rule, before: inspect(before_term), after: inspect(after_term)}
        Process.put(:cairn_pre_normalize_rewrites, [event | events])
