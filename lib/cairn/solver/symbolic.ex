defmodule Cairn.Solver.Symbolic do
  ...
  """
  @spec execute([Cairn.Types.token()], [Formula.sym_val()], map()) :: result()
  ...
  """
  @spec build_initial_stack([Cairn.Types.cairn_type()], map()) ::
  ...
  defp walk([{:ident, name, _} | rest], stack, env, depth) do
    case Map.get(env, name) do
      %Cairn.Types.Function{} = func when depth < @max_inline_depth ->
  ...
  defp append_trace_event(event) do
    events = Process.get(:cairn_prove_trace_events, [])
    Process.put(:cairn_prove_trace_events, [event | events])
  end
