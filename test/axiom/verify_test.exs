defmodule Axiom.VerifyTest do
  ...
  describe "VERIFY parsing" do
    test "lexer tokenizes VERIFY keyword" do
      assert {:ok, [{:verify_kw, "VERIFY", 0}, {:ident, "foo", 1}, {:int_lit, 100, 2}]} =
               Axiom.Lexer.tokenize("VERIFY foo 100")
    end

    test "parser produces verify item" do
      {:ok, tokens} = Axiom.Lexer.tokenize("VERIFY foo 100")
    ...
    test "parser handles VERIFY after function def" do
      ...
      assert %Axiom.Types.Function{name: "double"} = hd(items)
  ...
  describe "VERIFY failing" do
    test "finds counterexample for bad POST" do
      ...
      assert_raise Axiom.ContractError, ~r/VERIFY bad_negate: FAILED/, fn ->
        Axiom.eval_with_env(source)
      end
    end
    ...
    test "finds counterexample for weak PRE" do
      ...
      assert_raise Axiom.ContractError, ~r/VERIFY weak_withdraw: FAILED/, fn ->
        Axiom.eval_with_env(source)
      end
    end
  ...
  describe "Safe Bank integration" do
    ...
    test "buggy withdraw fails verification" do
      ...
      """
      assert_raise Axiom.ContractError, ~r/VERIFY withdraw_buggy: FAILED/, fn ->
        Axiom.eval_with_env(source)
      end
    end
  end
