defmodule Cairn.ConcurrencyRuntimeTest do
  ...
  test "runtime example runs end-to-end" do
    output =
      ExUnit.CaptureIO.capture_io(fn ->
        assert {[], _env} = Cairn.eval_file("examples/concurrency/ping_once.crn")
        Process.sleep(30)
      end)
  ...
  test "protocol example loads cleanly in file mode" do
    assert {[], _env} = Cairn.eval_file("examples/concurrency/protocol_ping_pong.crn")
  end

  test "SELF can send a bootstrap message to the current actor" do
    output =
      ExUnit.CaptureIO.capture_io(fn ->
        assert {[], _env} = Cairn.eval_file("examples/concurrency/self_boot.crn")
        Process.sleep(30)
      end)
  ...
  test "implicit actor-local RECEIVE can process multiple messages without pid juggling" do
    output =
      ExUnit.CaptureIO.capture_io(fn ->
        assert {[], _env} = Cairn.eval_file("examples/concurrency/two_pings.crn")
        Process.sleep(30)
      end)
  ...
  test "stateful counter actor can carry stack state across repeated receives" do
    output =
      ExUnit.CaptureIO.capture_io(fn ->
        assert {[], _env} = Cairn.eval_file("examples/concurrency/counter.crn")
        Process.sleep(30)
      end)
  ...
  test "traffic light actor can step through named phases" do
    output =
      ExUnit.CaptureIO.capture_io(fn ->
        assert {[], _env} = Cairn.eval_file("examples/concurrency/traffic_light.crn")
        Process.sleep(30)
      end)
  ...
  test "practical notifier actor can report queued and sent states" do
    output =
      ExUnit.CaptureIO.capture_io(fn ->
        assert {[], _env} = Cairn.eval_file("examples/concurrency/notifier.crn")
        Process.sleep(30)
      end)
  ...
  test "SPAWN_LINK propagates child EXIT to the linked caller" do
    ...
    {pid, ref} =
      spawn_monitor(fn ->
        Cairn.eval_file("examples/concurrency/linked_failure.crn")
        Process.sleep(100)
  ...
  test "restart example can observe failure and complete a one-shot restart" do
    output =
      ExUnit.CaptureIO.capture_io(fn ->
        assert {[], _env} = Cairn.eval_file("examples/concurrency/restart_once.crn")
      end)
  ...
  test "supervisor/worker example reports both observed exits explicitly" do
    output =
      ExUnit.CaptureIO.capture_io(fn ->
        assert {[], _env} = Cairn.eval_file("examples/concurrency/supervisor_worker.crn")
      end)
  ...
  test "binary-search guesser can converge on the referee target and finish" do
    output =
      ExUnit.CaptureIO.capture_io(fn ->
        assert {[], _env} = Cairn.eval_file("examples/concurrency/guess_binary.crn")
        Process.sleep(30)
      end)
  ...
  test "protocol mismatch example fails with a static error" do
    assert_raise Cairn.StaticError, ~r/SEND under protocol expects Ping, got Pong/, fn ->
      Cairn.eval_file("examples/concurrency/protocol_mismatch.crn")
    end
  end
end
