defmodule Cairn.ConcurrencyTypesTest do
  use ExUnit.Case
  ...
  test "pid types parse in signatures and recursive type fields" do
    ...
    assert [%Cairn.Types.TypeDef{name: "msg"}, %Cairn.Types.Function{name: "relay"}] = items
  end
  ...
  test "concurrency examples load successfully" do
    assert {[], _env} = Cairn.eval_file("examples/concurrency/ping_pong_types.crn")
    assert {[], _env} = Cairn.eval_file("examples/concurrency/protocol_ping_pong.crn")
    assert {[], _env} = Cairn.eval_file("examples/concurrency/traffic_light_types.crn")
