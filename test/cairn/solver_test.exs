defmodule Cairn.SolverTest do
  use ExUnit.Case, async: true

  alias Cairn.Solver.{Formula, Symbolic, SmtLib, Z3, Prove}
  ...
  describe "Prove.prove — unsupported operations" do
    ...
    test "MATCH on result type can be disproven" do
      ...
      assert_raise Cairn.ContractError, ~r/PROVE bad_match: DISPROVEN/, fn ->
        Cairn.eval(source)
      end
    end
  end
  ...
  describe "Prove.prove — option MATCH (v0.6.0a slice)" do
    test "proves unwrap_or_abs is non-negative" do
      ...
      output = ExUnit.CaptureIO.capture_io(fn -> Cairn.eval(source) end)
  ...
  describe "Prove.prove — generic ADT MATCH (v0.6.0c slice)" do
        ...
        ExUnit.CaptureIO.capture_io(:stderr, fn ->
          stdout =
            ExUnit.CaptureIO.capture_io(fn ->
              Cairn.eval_with_env(source, %{"__prove_trace__" => :summary})
            end)
    ...
    test "trace verbose mode includes candidate details" do
      ...
      stderr =
        ExUnit.CaptureIO.capture_io(:stderr, fn ->
          Cairn.eval_with_env(source, %{"__prove_trace__" => :verbose})
        end)
    ...
    test "trace json mode emits lifecycle + MATCH decision events to stderr" do
      ...
      stderr =
        ExUnit.CaptureIO.capture_io(:stderr, fn ->
          stdout =
            ExUnit.CaptureIO.capture_io(fn ->
              Cairn.eval_with_env(source, %{"__prove_trace__" => :json})
            end)
  ...
  describe "PROVE stability gates (v0.6.0ac)" do
    test "all_proven bundle stays within relaxed runtime budget" do
      all_proven_path = Path.expand("examples/prove/all_proven.crn")
      t0 = System.monotonic_time(:millisecond)

      stdout =
        ExUnit.CaptureIO.capture_io(fn ->
          Cairn.eval_file(all_proven_path)
        end)
  ...
  # ============================================================
  # Full pipeline integration tests (via Cairn.eval)
  # ============================================================
  ...
  describe "full PROVE pipeline — via Cairn.eval" do
    ...
    test "PROVE generic ADT failure prints decoded constructor counterexample" do
      ...
      assert_raise Cairn.ContractError, ~r/counterexample: p0 = (Circle|Square)\(-?[0-9]+\)/, fn ->
        Cairn.eval(source)
      end
    end
  ...
  describe "Lexer — PROVE keyword" do
    test "tokenizes PROVE" do
      {:ok, tokens} = Cairn.Lexer.tokenize("PROVE deposit")
  ...
  describe "Parser — PROVE statement" do
    ...
    test "PROVE without name is error" do
      {:ok, tokens} = Cairn.Lexer.tokenize("PROVE")
      assert {:error, _} = Cairn.Parser.parse(tokens)
    end

    test "expression splitting works with PROVE" do
      {:ok, tokens} = Cairn.Lexer.tokenize("1 2 ADD PROVE foo")
      {:ok, items} = Cairn.Parser.parse(tokens)
