defmodule Cairn.SumTypeTest do
  use ExUnit.Case
  ...
  defp define(source) do
    {_stack, env} = Cairn.eval_with_env(source)
    env
  end

  defp run(source, env) do
    {stack, _env} = Cairn.eval_with_env(source, env)
    stack
  end
  ...
  describe "Parser: TYPE declarations" do
    test "nullary constructors only" do
      ...
      assert [%Cairn.Types.TypeDef{name: "direction", variants: variants}] = items
    ...
    test "constructor with fields" do
      ...
      assert [%Cairn.Types.TypeDef{name: "option", variants: variants}] = items
    ...
    test "multiple constructors with fields" do
      ...
      assert [%Cairn.Types.TypeDef{} = td] = items
  ...
  describe "Evaluator: variant construction" do
    ...
    test "unknown constructor raises StaticError (caught by checker)" do
      ...
      assert_raise Cairn.StaticError, fn ->
        run("Blah", env)
      end
    end
  end
