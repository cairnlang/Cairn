defmodule Mix.Tasks.Axiom.RunTest do
  ...
  test "prints categorized examples index" do
    ...
    assert output =~ "examples/math.ax"
    assert output =~ "examples/strings.ax"
    assert output =~ "examples/interop.ax"
    ...
    assert output =~ "examples/concurrency/notifier.ax"
    assert output =~ "examples/concurrency/restart_once.ax"
    assert output =~ "examples/concurrency/supervisor_worker.ax"
    assert output =~ "examples/concurrency/guess_binary.ax"
