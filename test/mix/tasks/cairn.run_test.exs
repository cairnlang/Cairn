defmodule Mix.Tasks.Cairn.RunTest do
  ...
  test "prints prelude discoverability banner with --show-prelude" do
    ...
    assert stderr =~ "session_put"
    assert stderr =~ "session_clear"
    assert stderr =~ "session_has_user"
    assert stderr =~ "guard_require_login"
    assert stderr =~ "guard_require_role"
    assert stderr =~ "html_escape"
    assert stderr =~ "http_text_unauthorized"
    assert stderr =~ "http_text_forbidden"
